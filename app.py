...
# إضافة الأعمدة بأمان تام دون أخطاء تطابق
migrations = [
    (
    ...
    "ALTER TABLE messages ADD COLUMN msg_type TEXT DEFAULT 'text'",
...
for mig in migrations:
  try:
    ...
    pass  # العمود موجود مسبقاً ولا توجد مشكلة
...
if not st.session_state["logged_in"]:
  ...
  st.info(
      "⚡ **دخول سريع للطوارئ (اختر حسابك مباشرة لو واجهت خطأ كلمة المرور):**"
  )
  ...
  if all_db_users:
    ...
    for idx, (db_u, db_av, db_b, db_st) in enumerate(all_db_users):
      ...
      if col_target.button(
          f"دخول كـ {db_av} {db_u}", key=f"quick_login_{db_u}"
      ):
        st.session_state["logged_in"] = True
        st.session_state["username"] = db_u
        st.session_state["avatar"] = db_av or "😀"
        st.session_state["bio"] = (
            db_b or "مرحباً، أنا أستخدم ماسنجر الأصدقاء!"
        )
        st.session_state["status"] = db_st or "online"
        st.rerun()
  else:
    st.caption(
        "لا توجد حسابات مخزنة بعد في قاعدة البيانات هذه. أنشئ حساباً جديداً"
        " أدناه."
    )
  ...
  with tab1:
    ...
    if st.button("دخول تقليدي"):
      c.execute(
          "SELECT avatar, bio, status, password_hash FROM users WHERE"
          " username=?",
          (l_user,),
      )
      user_row = c.fetchone()
      if user_row:
        ...
        st.session_state["avatar"] = db_av or "😀"
        st.session_state["bio"] = (
            db_bio or "مرحباً، أنا أستخدم ماسنجر الأصدقاء!"
        )
    ...
    if st.button("إنشاء الحساب"):
      if not r_user or not r_pass:
        ...
        try:
          ...
          conn.commit()
          st.success(
              "تم إنشاء الحساب بنجاح! انتقل لتبويب تسجيل الدخول أو استخدم"
              " الدخول السريع أعلى."
          )
    ...
    with st.container():
      ...
      if media_file is not None:
        ...
        if st.session_state.get("last_media_hash") != file_hash:
          ...
          current_time = datetime.datetime.now().strftime("%H:%M")
          c.execute(
              "INSERT INTO messages (sender, receiver, avatar, msg_type,"
              " content, ts, is_read) VALUES (?, ?, ?, ?, ?, ?, 0)",
              (
                  str(cur_user),
                  str(target_friend),
                  str(st.session_state.get("avatar", "😀")),
